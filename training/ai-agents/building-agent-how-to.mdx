...
Crucially, every time a message is received at a Milestone, **Cortana AI uses AI Reasoning models to analyze the user's intent**. This isn't a simple keyword match; the AI reasons through the context of the conversation to intelligently decide which path is the best one to send the conversation towards.
...
The Wait node provides fine-tuned control for specific points *within* a workflow. However, Cortana also has a global **Message Delay** setting. This global setting introduces a pause *before every* AI response to a user's message. Using the global delay is often more efficient than adding a Wait node between every single conversational node.
...
The "Stop" node is a terminal node in Cortana that serves a critical function: to completely and definitively end an agent's interaction with a contact. When a workflow reaches a Stop node, it ceases all communication and unassigns itself from the user across all platforms.
