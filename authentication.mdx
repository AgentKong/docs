...
description: "Create and manage API keys for the Agent Kong API and MCP Server"
---
...
<Steps>
  <Step title="Open Settings">
    Navigate to **Settings → API Keys** in your Agent Kong dashboard.
  </Step>
...
curl -H "Authorization: Bearer sk-ak-your-api-key" \
  https://agent-kong-app.vercel.app/api/v1/businesses/{businessId}/contacts
```
...
```
https://app.agentkong.com/business/cf3383a0-55f6-45d8-ab57-484edd63770c/tracking
                                     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
